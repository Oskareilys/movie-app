...
const getOne = catchError(async(req, res) => {
    ...
    if(!result) return res.status(404).json( {Message: "the movie not found"} );
...
const update = catchError(async(req, res) => {
    ...
    if(result[0] === 0) return res.status(404).json( {Message: "the movie not found"} );
...
const setMovieGenres = catchError(async(req, res) => {
    ...
    if(!movie) return res.status(404).json( {Message: "the movie not found"} );
